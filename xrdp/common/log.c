...
/* produce a hex dump */
	void DEFAULT_CC
log_hexdump(struct log_config* l_cfg, const char* _file, const int _line, const unsigned int lvl, unsigned char *p, unsigned int len)
{
	...
	while (offset < len)
	{
		...
		dump_offset = 0;
		log_message(l_cfg, _file, _line, lvl, dump );
		dump_offset = dump;
		offset += thisline;
		line += thisline;
	}
